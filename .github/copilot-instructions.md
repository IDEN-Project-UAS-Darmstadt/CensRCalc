...
package supports both scenarios with and without recruitment/accrual time.
We use the instructions by Hadley Wickham and Jennifer Bryan in 
"R Packages" (https://r-pkgs.org/) for best practices in R package 
development. We do not follow the tidyverse style guide. Everything should 
be up to scientific standards and use the language of the scientific 
community around survival analysis and simulation studies.
...
Make sure that there are no potential pitfalls in the code that
could lead to errors or unexpected behavior. Pay special attention to 
...
the `Rdpack` package. For normal citations: `\insertCite{citkey1,citkey2}{CensRCalc}`,
and for textual citations: `\insertCite{citkey1,citkey2;textual}{CensRCalc}`.
...
\ lead to problems when rendering to PDF and pkgdown, do not use cases, instead
use arrays.

The title of an R function should not contain repetitions also used in other
titles in the package. They should describe actions or computational procedures. 
Two examples: "Calculate censoring parameter for exponential distribution" and 
"find_cens_prop(): Find censoring rate for target proportion"
